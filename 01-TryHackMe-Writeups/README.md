# TryHackMe Learning Log

This folder contains documentation and write-ups from my practice labs on TryHackMe.

### My Cybersecurity Learning Master Plan

I have designed a comprehensive learning curriculum that combines operational depth (SOC) with technical architectural expertise (Engineering).

#### Phase 1: The Foundation
- [ ] **Pre-Security** - *In Progress*
- [ ] Cyber Security 101

#### Phase 2: Security Analyst Track
- [ ] SOC Level 1
- [ ] Security Analyst Level 1 (SAL1)
- [ ] SOC Level 2
- [ ] Advanced Endpoint Investigations

#### Phase 3: Security Engineer Track
- [ ] Security Engineer
- [ ] DevSecOps
- [ ] Attacking and Defending AWS

#### Phase 4: Penetration Tester (Optional)
- [ ] Jr Penetration Tester
- [ ] Jr Penetration Tester (PT1)
- [ ] Web Fundamentals

---

## Completed Rooms & Write-ups

| Date | Room Name | Path | Write-up Link |
| :--- | :--- | :--- | :--- |
| 19 Jan 2026 | **Offensive Security Intro** | Pre-Security | [Read Report](./pre-security/offensive-security-intro.md) |
| 19 Jan 2026 | **Defensive Security Intro** | Pre-Security | [Read Report](./pre-security/defensive-security-intro.md) |
| 19 Jan 2026 | **Careers in Cyber** | Pre-Security | [Read Report](./pre-security/careers-in-cyber.md) |
| 20 Jan 2026 | **What is Networking?** | Pre-Security | [Read Report](./pre-security/what-is-networking.md) |
| 20 Jan 2026 | **Intro to LAN** | Pre-Security | [Read Report](./pre-security/intro-to-lan.md) |
| 21 Jan 2026 | **OSI Model** | Pre-Security | [Read Report](./pre-security/osi-model.md) |
| 21 Jan 2026 | **Packets & Frames** | Pre-Security | [Read Report](./pre-security/packetsframes.md) |
| 21 Jan 2026 | **Extending Your Network** | Pre-Security | [Read Report](./pre-security/extending-your-network.md) |
| 22 Jan 2026 | **DNS in Detail** | Pre-Security | [Read Report](./pre-security/dns-in-detail.md) |
| 22 Jan 2026 | **HTTP in Detail** | Pre-Security | [Read Report](./pre-security/http-in-detail.md) |
| 23 Jan 2026 | **How Websites Work** | Pre-Security | [Read Report](./pre-security/how-websites-work.md) |
| 23 Jan 2026 | **Putting it All Together** | Pre-Security | [Read Report](./pre-security/putting-it-all-together.md) |
| 24 Jan 2026 | **Linux Fundamental Part 1** | Pre-Security | [Read Report](./pre-security/linux-fundamental-part-1.md) |

## 📝 Documentation Format
Each lab report follows this structure:
1.  **Objective:** The learning goal of the room.
2.  **Tools:** Tools used (e.g., Wireshark, Splunk, Nmap).
3.  **Key Steps:** Technical steps taken and important commands.
4.  **Analysis:** My analysis of the findings (flags/vulnerabilities).
